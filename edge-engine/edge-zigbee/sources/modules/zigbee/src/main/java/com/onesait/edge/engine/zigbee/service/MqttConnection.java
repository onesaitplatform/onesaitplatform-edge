...
public class MqttConnection implements MqttCallback {
	...
	public static final String CLIENT_ID = "Zigbee";
...
//			LOGGER.info("Broker not found!! Using tcp://{}:{}", mqttHost, mqttPort);

			try {

				// Delete connection files
				LOGGER.debug("MQTT Cleaning temporal files : {}", this.cleanFiles());
...
//				LOGGER.error("MQTT CONNECTION ERROR: {}; CAUSE: {}", brokerUrl, e.getMessage());
	...
	public void sendThroughMqtt(List<MqttMsgDetail> mqttmsgs) {
		if((mqttmsgs!=null)&&(!mqttmsgs.isEmpty())){
		for (MqttMsgDetail msg : mqttmsgs) {
			List<String> jsonArray = new ArrayList<>();
			String jsonResponse = getObjectAsJson(msg);
			jsonArray.add(jsonResponse);
			sendSignal(jsonArray, getTopicSignalsIn());
		}
		}
	...
	private String getObjectAsJson(Object object) {
		...
		try {
			json = mapper.setSerializationInclusion(Include.NON_EMPTY).writerWithDefaultPrettyPrinter().writeValueAsString(object);
		} catch (Exception e) {
			json = "";
		}
...
//	}
	...
	private synchronized void sendSignal(List<String> msg, String topic) {
		if (this.getMqttClient() != null && this.getMqttClient().isConnected()) {
			// Mandar datos por mqtt
